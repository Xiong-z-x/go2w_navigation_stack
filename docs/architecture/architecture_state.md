...
## Current Phase
- Active Phase: `Phase 4C-min`
- Phase Status: Phase 4C-min is accepted on the current `main` baseline as the smallest flat/stair/flat execution gate above the Phase 4B-min mission segment runtime. Phase 1, Phase 2, Phase 3A, Phase 3B, Phase 3C, Phase 4 pre-migration handoff, Phase 4A, and Phase 4B-min remain accepted baselines. Phase 4C-min proves manual route computation, flat/stair/flat segment decomposition, navigation-owned `NavigateToPose` flat segment dispatch, dedicated `/stair_exec` Action dispatch for stair segments, and stable mission-level diagnostics for flat success, flat failure, flat cancellation, flat timeout, flat Action unavailable, stair outcomes, route unavailable, and connector unavailable. It does not implement production mission orchestration, real Nav2 route tracking against robot motion, real stair locomotion, real cross-floor autonomy, elevation mapping, traversability, automatic stair detection, automatic connector generation, `map_server`, AMCL, or `map -> odom` localization.
...
- `go2w_mission` now contains a Phase 4A handoff demo client and a Phase 4C-min one-shot mission segment runtime used to prove route segmentation, flat `NavigateToPose` dispatch, and `/stair_exec` dispatch. It does not contain a production Mission Orchestrator.
...
- Repository-local FAST-LIO integration is accepted through Phase 3C. Current work covers pinned external-source patching, build validation, no-TF dry-run automation, local input/output contract adapters, perception-side `odom -> base_link` TF activation, longer-window perception output stability, a standalone Nav2 costmap consumer gate, a minimal same-floor Nav2 closed loop, a minimal same-floor `nav2_route` graph baseline, Phase 3C multi-floor map/route/world hardening assets, the Phase 4A staircase handoff skeleton, the Phase 4B-min mission segment runtime, and the Phase 4C-min flat/stair/flat execution gate.

## Accepted Work and Only Allowed Next Task
- The current project state is formally `Phase 4C-min` accepted. Any next implementation still requires a separate complete task card or an explicitly self-approved task card under the current autonomous Phase 4 execution authorization.
