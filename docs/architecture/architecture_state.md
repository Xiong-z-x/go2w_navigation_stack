...
## Current Phase
- Active Phase: `Phase 4A`
- Phase Status: Phase 4A is accepted on the current `main` baseline as the minimal staircase state-machine / control-ownership handoff skeleton. Phase 1, Phase 2, Phase 3A, Phase 3B, Phase 3C, and Phase 4 pre-migration handoff remain accepted baselines. Phase 4A proves manual staircase connector detection, dedicated `/stair_exec` Action reachability, command ownership mutual exclusion, and success/failure/cancel/timeout diagnostics. It does not implement production mission orchestration, real stair locomotion, real cross-floor autonomy, elevation mapping, traversability, automatic stair detection, automatic connector generation, `map_server`, AMCL, or `map -> odom` localization.
...
- `go2w_mission` now contains a Phase 4A handoff demo client and launch path used to prove route connector detection and `/stair_exec` handoff. It does not contain a production Mission Orchestrator.
