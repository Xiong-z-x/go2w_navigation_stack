...
- `go2w_control` now contains the Phase 4A `StairExec` Action definition, command gate, explicit owner-to-motion-mode state, static Go2W motion profiles, a startup stand initializer, and the minimal stair executor skeleton. The stair executor policy now reuses the legged motion profile, clamps stair velocity to that conservative baseline, and publishes a 12-joint leg hold command while stair ownership is active. It does not contain a real stair locomotion controller.
...
- The opt-in Go2W real model / motion-mode baseline has been added and verified: the real-model launch starts headless, the four-foot-wheel `diff_drive_controller`, leg position controller, sensors, joint states, and stand initializer pass `tools/verify_go2w_real_model_baseline.sh`.
