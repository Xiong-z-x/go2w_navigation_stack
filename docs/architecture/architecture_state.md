...
- The next implementation boundary is a separate complete task card for the smallest useful post-Phase-4 step, such as real-model route-following expansion, terrain-aware connector discovery, or a production-grade mission API skeleton. It must not include real staircase traversal controller tuning, real multi-floor autonomous behavior, elevation mapping, traversability, automatic stair connector generation, default real-model re-baselining, or any change to the perception-owned `odom -> base_link` authority unless those items are explicitly named and verified in that task card.
