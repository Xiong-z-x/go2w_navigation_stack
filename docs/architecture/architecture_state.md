...
## Current Phase
- Active Phase: `Phase 4B-min`
- Phase Status: Phase 4B-min is accepted on the current `main` baseline as the smallest mission-side route segmentation and staircase dispatch runtime above the Phase 4A handoff skeleton. Phase 1, Phase 2, Phase 3A, Phase 3B, Phase 3C, Phase 4 pre-migration handoff, and Phase 4A remain accepted baselines. Phase 4B-min proves manual route computation, flat/stair/flat segment decomposition, dedicated `/stair_exec` Action dispatch for stair segments, and stable mission-level diagnostics for success, failure, cancellation, timeout, route unavailable, and connector unavailable. It does not implement production mission orchestration, real flat-ground Nav2 route execution, real stair locomotion, real cross-floor autonomy, elevation mapping, traversability, automatic stair detection, automatic connector generation, `map_server`, AMCL, or `map -> odom` localization.
...
- `go2w_mission` now contains a Phase 4A handoff demo client and a Phase 4B-min one-shot mission segment runtime used to prove route segmentation and `/stair_exec` dispatch. It does not contain a production Mission Orchestrator.
...
- Repository-local FAST-LIO integration is accepted through Phase 3C. Current work covers pinned external-source patching, build validation, no-TF dry-run automation, local input/output contract adapters, perception-side `odom -> base_link` TF activation, longer-window perception output stability, a standalone Nav2 costmap consumer gate, a minimal same-floor Nav2 closed loop, a minimal same-floor `nav2_route` graph baseline, Phase 3C multi-floor map/route/world hardening assets, the Phase 4A staircase handoff skeleton, and the Phase 4B-min mission segment runtime.

## Accepted Work and Only Allowed Next Task
- The current project state is formally `Phase 4B-min` accepted. Any next implementation still requires a separate complete task card.
...
- The next implementation boundary is a separate complete task card for the smallest useful post-Phase-4B-min step, such as wiring a real flat segment executor around an existing Nav2 runtime or adding a stronger route-tracking observation gate. It must not include production Mission Orchestrator scope, real staircase traversal controller tuning, real multi-floor autonomous behavior, elevation mapping, traversability, automatic stair connector generation, or any change to the perception-owned `odom -> base_link` authority unless those items are explicitly named and verified in that task card.
