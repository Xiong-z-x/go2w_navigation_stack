...
## Current Phase
- Active Phase: `Phase 4D-min`
- Phase Status: Phase 4D-min is accepted on the current `main` baseline as the smallest route-tracking feedback observation gate above the Phase 4C-min flat/stair/flat execution gate. Phase 1, Phase 2, Phase 3A, Phase 3B, Phase 3C, Phase 4 pre-migration handoff, Phase 4A, Phase 4B-min, and Phase 4C-min remain accepted baselines. Phase 4D-min proves that mission-side logic can consume the standard `nav2_msgs/action/ComputeAndTrackRoute` feedback shape, observe staircase edge `500`, detect a `stair_exec` route operation trigger, and diagnose missing feedback operation or unavailable route-tracking action paths. It does not implement production mission orchestration, real Nav2 route tracking against robot motion, a real `nav2_route` operation plugin, real stair locomotion, real cross-floor autonomy, elevation mapping, traversability, automatic stair detection, automatic connector generation, `map_server`, AMCL, or `map -> odom` localization.
...
- `go2w_navigation` now contains a Phase 2H standalone Nav2 costmap consumer config and launch, a Phase 3A minimal same-floor Nav2 planner/controller/BT bringup, a Phase 3B minimal `nav2_route` route server plus manual same-floor GeoJSON route graph, Phase 3C persistent multi-floor route graph / map metadata assets, a Phase 4C-min flat navigation executor skeleton for `NavigateToPose` verifier dispatch, and a Phase 4D-min route tracking feedback executor skeleton for `ComputeAndTrackRoute` feedback observation.
- `go2w_control` now contains the Phase 4A `StairExec` Action definition, command gate, and minimal stair executor skeleton. It does not contain a real stair locomotion controller.
- `go2w_mission` now contains a Phase 4A handoff demo client, a Phase 4C-min one-shot mission segment runtime used to prove route segmentation, flat `NavigateToPose` dispatch, and `/stair_exec` dispatch, plus a Phase 4D-min route tracking feedback observer. It does not contain a production Mission Orchestrator.
...
- The current project state is formally `Phase 4D-min` accepted. Any next implementation still requires a separate complete task card or an explicitly self-approved task card under the current autonomous Phase 4 execution authorization.
...
- The next implementation boundary is a separate complete task card for the smallest useful post-Phase-4D-min step, such as a Phase 4 aggregate acceptance gate or a production-grade mission API skeleton. It must not include real staircase traversal controller tuning, real multi-floor autonomous behavior, elevation mapping, traversability, automatic stair connector generation, or any change to the perception-owned `odom -> base_link` authority unless those items are explicitly named and verified in that task card.
- Forbidden in the next task unless explicitly approved by its task card: Unitree Go2W model import, production mission orchestration, real staircase traversal controller tuning, multi-floor autonomous behavior, elevation mapping, traversability, automatic stair connector generation, `map_server` / AMCL / `map -> odom` localization, and any change to the perception-owned `odom -> base_link` authority.
