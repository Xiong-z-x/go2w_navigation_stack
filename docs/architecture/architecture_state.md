...
- `go2w_control` now contains the Phase 4A `StairExec` Action definition, command gate, explicit owner-to-motion-mode state, static Go2W motion profiles, a startup stand initializer, and the minimal stair executor skeleton. It does not contain a real stair locomotion controller.
