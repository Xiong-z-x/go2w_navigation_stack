...
- Unitree ROS 2 Go2 public examples expose `body_height`, `foot_raise_height`, and `gait_type` in the sport-mode state, including `body height: 0.320000`, `raise height: 0.090000`, and gait type labels such as trot and climb-stair. The current profile values are recorded as conservative simulation metadata, not as hardware-certified gait tuning.
...
## Verification Run
- Date: `2026-05-02T01:08:11+08:00`
- Command: `./tools/verify_go2w_real_model_baseline.sh`
- Evidence directory: `/tmp/go2w_real_model_baseline_27364`
...
- `joint_state_broadcaster`, `leg_position_controller`, and `diff_drive_controller` all reach `active`, and the verifier now waits on controller-state polling rather than trusting a single spawner log line.
...
- `go2w_stand_initializer` publishes a 12-value stand command to `/leg_position_controller/commands` and logs the selected `legged` profile summary before publishing.
...
stand_initializer_profile_legged: PASS
stand_initializer_result_legged: PASS
clock_message: PASS
imu_message: PASS
...
- `sim_go2w_real.launch.py` now passes `--motion-mode legged` to the stand initializer so the startup posture explicitly records the selected profile.
