...
三、当前阶段事实：
当前正式阶段是已验收的 Phase 4C-min。Phase 1、Phase 2、Phase 3A、Phase 3B、
Phase 3C、Phase 4 迁移前封板、Phase 4A、Phase 4B-min 和 Phase 4C-min 均已有仓库内验收证据。
Phase 4C-min 只完成最小 flat/stair/flat execution gate，不代表 production Mission
...
- go2w_navigation 只管 Nav2、costmap、planner/controller、route server；当前还有 Phase 4C-min flat navigation executor skeleton。
- go2w_mission 只管目标语义、楼层语义和任务分段；当前有 Phase 4A handoff demo、Phase 4B-min one-shot mission segment runtime 和 Phase 4C-min flat/stair/flat gate。
...
6. 读取 architecture_state.md 的 Accepted Work and Only Allowed Next Task。
7. 如果要继续 Phase 4C-min 之后的任务，先生成完整 6 项任务单或自批准任务单。
8. 只做一个最小任务，不混入真实楼梯控制、自动连接器、AMCL 或模型导入。
