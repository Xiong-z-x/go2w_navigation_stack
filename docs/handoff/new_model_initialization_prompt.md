...
三、当前阶段事实：
当前正式阶段是已验收的 Phase 4A。Phase 1、Phase 2、Phase 3A、Phase 3B、
Phase 3C、Phase 4 迁移前封板和 Phase 4A 均已有仓库内验收证据。Phase 4A
只完成最小楼梯状态机/控制权交接骨架，不代表真实跨楼层自主导航。
...
Phase 4A 之后的任务不应顺手扩展成 production mission orchestration、真实爬楼
...
- go2w_mission 只管目标语义、楼层语义和任务分段；当前只有 Phase 4A handoff demo。
...
4. 读取 architecture_state.md 的 Accepted Work and Only Allowed Next Task。
5. 如果要继续 Phase 4A 之后的任务，先生成完整 6 项任务单。
6. 只做一个最小任务，不混入真实楼梯控制、自动连接器、AMCL 或模型导入。
