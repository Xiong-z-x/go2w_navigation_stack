...
- `go2w_mission`：Phase 4A 已新增 handoff demo 和最小 launch 验证路径；
  ...
  非抢占式 queued priority scheduling、local assignment policy 和 operator workflow-policy snapshot；当前 priority 只影响 waiting queue，不抢占 active mission；
...
- 未实现完整 production Mission Orchestrator 的多机器人调度优化、cross-robot goal transfer 和长期 workflow backend；当前 assignment policy 只是本机 admission gate，workflow policy 只是只读 snapshot 与诊断视图。
