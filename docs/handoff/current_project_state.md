...
- `go2w_navigation`：Phase 2H costmap gate、Phase 3A Nav2 同层闭环、
  ...
  Nav2 params file、route-following verifier 和 real-model `nav2_route`
  robot-motion route-tracking verifier。
- `go2w_mission`：Phase 4A 已新增 handoff demo 和最小 launch 验证路径；
  ...
  opt-in real-model flat-only execution gate，并可在显式 `route_tracking_action`
  配置下让 flat segment 同步观察真实 `ComputeAndTrackRoute` feedback；当前还新增
  opt-in real-model flat/stair/flat integration gate，在同一 `RunMission` 中用真实
  Nav2 执行 flat edge `10` 和 `20`，用 `/stair_exec` skeleton 执行 handoff edge
  `500`，并观察 route tracking feedback、stair phase sequence 和 command gate
  owner/mode 切换。
...
- Go2W real model `nav2_route` robot-motion route-tracking verifier：通过
  ...
  odom route graph、`ComputeAndTrackRoute` feedback edge `10`、`NavigateToPose`
  success、`/cmd_vel`、perception odom motion、diff-drive odom motion，以及
  absent `map -> odom`。
- Mission runtime real-model flat execution + route-tracking observation gate：通过
  ...
  `/navigate_to_pose`，同时启用 mission-side `/compute_and_track_route`
  observation 并观察 edge `10`；该 gate 保留 route graph 目标 yaw，并观察到
  `MISSION_SUCCEEDED`、非零 `/cmd_vel`、perception odom motion、diff-drive odom
  motion 和 perception-owned `odom -> base_link`。
- Mission runtime real-model flat/stair/flat integration gate：通过
  `tools/verify_go2w_mission_real_flat_stair_flat.sh` 验证 `RunMission` 在不启动
  `go2w_flat_nav_executor` 的情况下执行 `flat:10;stair:500:stair_a:F1->F2;flat:20`；
  flat segments 走真实 Nav2 `/navigate_to_pose`，stair segment 走 dedicated
  `/stair_exec` skeleton，并观察 mission-side route feedback edges `10` / `20`、
  stair phase sequence、command gate `flat/wheeled -> stair/legged -> flat/wheeled`、
  非零 `/cmd_vel`、perception/diff-drive odom motion 和 TF 边界。该 gate 仍不是
  真实楼梯动力学、真实 stair route operation plugin 或 production cross-floor route tracking。
...
- Phase 4C-min 的 flat executor 仍作为 deterministic verifier skeleton 保留；mission API
  现在已有 opt-in real-model flat-only gate 和 opt-in flat/stair/flat integration gate
  可绕过该 skeleton 并调用真实 Nav2 `/navigate_to_pose`，也可显式启用
  `/compute_and_track_route` observation。
- 同层 real-model route-following、real-model route-tracking observation、mission
  real-model flat execution / route-tracking observation 和 mission real-model
...
- Mission flat-only integration of real `nav2_route` robot-motion route tracking 和
  mission real-model flat/stair/flat integration gate 已完成；production cross-floor
  route tracking、真实 stair route operation plugin 和真实楼梯动力学仍应作为后续独立任务处理，
  不要和 production Mission Orchestrator 调度策略、AMCL/map_server 或 Phase 5 自动连接器混在同一实现窗口。
