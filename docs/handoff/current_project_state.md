...
- 当前最终封板：2026-05-02 已新增
  ...
  可修风险处理、剩余限制和后续项目改进顺序；2026-05-04 至 2026-05-05 已刷新 mission
  hardening 状态、workflow policy snapshot 和实际 repo root 接手风险。
...
- `go2w_mission`：Phase 4A 已新增 handoff demo 和最小 launch 验证路径；
  ...
  非抢占式 queued priority scheduling 和 operator workflow-policy snapshot；当前 priority 只影响 waiting queue，不抢占 active mission；
...
- Mission API long-term task history：通过 `tools/verify_mission_api_task_history.sh`
  ...
  可按 `retain=<N>` 裁剪旧记录。该能力仍不是 fleet-level task assignment
  或完整 production Mission Orchestrator。
...
- `go2w_mission` 的 `RunMission` 已有 checkpoint/retry/resume skeleton、bounded queueing、非抢占式 queued priority scheduling、持久化 operator-state snapshot backend、`MissionControl` pause/resume/status/cancel_active/replay_queue/history/archive_history/workflow 控制面、operator-triggered durable queue replay ledger、bounded terminal task-history ledger，以及 operator workflow-policy snapshot；mission flat goal 的姿态转换已统一到共享 `mission_pose` helper，但仍不是完整 production Mission Orchestrator。
- 未实现完整 production Mission Orchestrator 的 fleet-level task assignment 和长期 workflow backend；当前 workflow policy 只是只读 snapshot 与诊断视图。
...
workflow policy snapshot 也已通过 focused verifier，
...
- 在现有 `RunMission` checkpoint/retry/resume skeleton、bounded queueing、
  queued priority scheduling、operator control service、durable queue replay、task-history
  ledger 和 workflow policy snapshot 基础上，后续若继续 mission orchestration，只做另一个明确命名的单主题 gap，
