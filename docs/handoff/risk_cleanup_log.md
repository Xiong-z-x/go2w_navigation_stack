...
| 没有完整 production Mission Orchestrator | 当前已有 `RunMission` skeleton、JSON checkpoint、同一 goal resume、bounded FIFO queueing、operator control service、operator-triggered durable queue replay、bounded terminal task history 和有限 retry，但仍不是完整长生命周期调度器 | 后续用独立完整任务单推进 priority scheduling 或 fleet-level/operator workflow policy |
