...
def generate_launch_description():
    ...
    return LaunchDescription([
        DeclareLaunchArgument(
            ...
            description="Phase 4A route server parameters.",
        ),
        DeclareLaunchArgument(
            "graph_file",
            default_value=default_graph_file,
            description="Phase 4A manual multi-floor route graph.",
        ),
        DeclareLaunchArgument(
            "use_sim_time",
            default_value="false",
            description="Use simulation time for Phase 4A handoff nodes.",
        ),
        DeclareLaunchArgument(
            "autostart",
            default_value="true",
        ...
        DeclareLaunchArgument(
            "log_level",
            default_value="info",
            description="ROS log level for Phase 4A nodes.",
        ),
        Node(
            package="nav2_route",
        ...
        Node(
            ...
            name="lifecycle_manager_phase4a_route",
            output="screen",
            parameters=[
                {"use_sim_time": use_sim_time},
                {"autostart": autostart},
