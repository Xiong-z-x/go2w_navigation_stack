...
def generate_launch_description():
    ...
    controller_yaml_path = os.path.join(sim_share, 'config', 'controllers_go2w_real.yaml')
    rviz_gpu_env = {
    ...
    return LaunchDescription([
        ...
        RegisterEventHandler(
            OnProcessExit(
                ...
                on_exit=[stand_initializer],
            )
        ),
        RegisterEventHandler(
            OnProcessExit(
                target_action=stand_initializer,
                on_exit=[rviz],
            )
        ),
    ])
